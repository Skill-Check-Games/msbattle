...
This is the old CLAUDE.md, kept in full as a reference. It is NOT auto-loaded into Claude sessions any more (it was ~44k tokens per request). The lean `CLAUDE.md` at the repo root points here; read the relevant section only when working on that area.
