...
test("a second connection authenticating as the same user mid-round reclaims the seat, without the old socket ever disconnecting first", async function() {
	...
	try {
		...
		var c2DrawBoard = once(c2, "draw_board", 20000);   // the first frame comes when the round goes live: FIRST_ROUND_START_DELAY_MS (8.3s) after start_game, plus slack for a loaded machine (the suites run in parallel)
