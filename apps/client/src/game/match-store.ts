...
export interface GameFrame { id: string; playerName: string; skin: string | null; avatar: string | null; country: string | null; state: number[][]; finished: boolean; finishedAt: number; safeCount: number; totalSafe: number; progress: number; frozenUntil: number; playing: boolean; }
...
class MatchStore {
	...
	private opponentCanvases = new Map<string, { canvas: HTMLCanvasElement; skin: string | null }>();
	private wired = false;
	...
	registerOpponentCanvas(id: string, canvas: HTMLCanvasElement | null, skin: string | null) { if (canvas) this.opponentCanvases.set(id, { canvas, skin }); else this.opponentCanvases.delete(id); }
	...
	wire() {
		...
		socket.on("ranked_searching", (info) => {
			...
			this.set({ search: { ...this.state.search, members: (info && info.members) || [], size: (info && info.size) || this.state.search.size } });
		});
		...
		socket.on("start_game", (d) => {
			sound.unlock();
			this.set({ roundResultShown: false, roundLive: false, frozenUntil: 0, roundResult: null, waitingCleared: false, gameProgress: formatGameProgress(d.gameNumber, d.gameCount, (this.state.room && this.state.room.scoreTarget) || d.scoreTarget) });
			this.lastFinished = {};
			...
			countDown(this.session, d.startDelayMs || 0, () => this.localRoundStartReveal(), { sound });
		});
		...
		socket.on("game_result", (d) => {
			this.set({ roundResultShown: true, frozenUntil: 0, roundDeadline: null, roundResult: { winnerId: d.winnerId, standings: d.standings || [] } });
	...
	private localRoundStartReveal() {
		...
		s.render();
		const targets: Array<{ canvas: HTMLCanvasElement; skin: string | null; state: number[][] }> = [];
		this.opponentCanvases.forEach(({ canvas, skin }) => targets.push({ canvas, skin: skin || "classic", state: initial }));
		if (targets.length) s.startOpponentRevealAnim(targets);
	}
