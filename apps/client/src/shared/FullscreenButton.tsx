...
export default function FullscreenButton({ className, lockLandscape }: { className?: string; lockLandscape?: boolean }) {
	...
	return (
		<button type="button" className={`${styles.btn} ${className || ""}`} title={on ? "Exit fullscreen" : "Fullscreen"} aria-label={on ? "Exit fullscreen" : "Fullscreen"} onClick={() => on ? exitGameFullscreen() : enterGameFullscreen(true, !!lockLandscape)}>
